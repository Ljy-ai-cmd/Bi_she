...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "\"%s\" lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    % (sys.executable, args.script, args.config, args.save_dir, args.use_lmdb)
    elif args.mode == "multiple":
        train_cmd = "\"%s\" -m torch.distributed.run --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d " \
                    % (sys.executable, args.nproc_per_node, args.master_port, args.script, args.config, args.save_dir, args.use_lmdb)
    else:
