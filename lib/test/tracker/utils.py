...
class Preprocessor(object):
    ...
    def process(self, img_arr: np.ndarray, enhance_contrast=False):
        # Handle infrared/single channel images
        if len(img_arr.shape) == 2:
            # Single channel grayscale image (H, W) -> (H, W, 3)
            img_arr = np.stack([img_arr, img_arr, img_arr], axis=2)
        elif img_arr.shape[-1] == 1:
            # Single channel with explicit dimension (H, W, 1) -> (H, W, 3)
            img_arr = np.repeat(img_arr, 3, axis=2)
        elif img_arr.shape[-1] == 6:
            # Multi-modal input (RGB + Thermal)
            mean = self.mm_mean
            std = self.mm_std
        
        # Enhance contrast for infrared images with heavy background interference
        if enhance_contrast and len(img_arr.shape) == 3:
            # Apply CLAHE (Contrast Limited Adaptive Histogram Equalization)
            lab = cv.cvtColor(img_arr.astype(np.uint8), cv.COLOR_RGB2LAB)
            l, a, b = cv.split(lab)
            clahe = cv.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
            l = clahe.apply(l)
            lab = cv.merge([l, a, b])
            img_arr = cv.cvtColor(lab, cv.COLOR_LAB2RGB)
        ...
        # Fix: Convert numpy array to float32 before creating tensor (PyTorch 2.0+ compatibility)
        img_tensor = torch.from_numpy(img_arr.astype(np.float32)).cuda().permute((2,0,1)).unsqueeze(dim=0)
