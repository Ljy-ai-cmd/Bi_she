...
class Tracker:
    ...
    def _read_image(self, image_file: str):
        if isinstance(image_file, str):
            ...
            if len(image_file) == 3:
                rgb_path, ir_path, modality = image_file
                ...
                ir = cv.cvtColor(ir, cv.COLOR_BGR2RGB)
                
                if rgb.shape[:2] != ir.shape[:2]:
                    ir = cv.resize(ir, (rgb.shape[1], rgb.shape[0]))
                ...
                raise ValueError("type of image_file should be str or list/tuple of length 2 or 3")
        else:
