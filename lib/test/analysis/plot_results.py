...
def _compute_fps(trackers, dataset, valid_sequence):
    special_datasets = ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uav', 'nfs', 'tnl2k',
                        'antiuav_rgb', 'antiuav_ir']
    seq_valid = valid_sequence.tolist()
    fps_list = []
    for trk in trackers:
        total_time = 0.0
        total_frames = 0
        for seq, is_valid in zip(dataset, seq_valid):
            if not is_valid:
                continue
            if seq.dataset in special_datasets:
        ...
        if total_time > 0 and total_frames > 0:
            fps_list.append(total_frames / total_time)
        else:
            fps_list.append(float('nan'))
    return torch.tensor(fps_list, dtype=torch.float32)
...
def print_results(trackers, dataset, report_name, merge_results=False,
    ...
    scores['FPS'] = _compute_fps(trackers, dataset, valid_sequence)
