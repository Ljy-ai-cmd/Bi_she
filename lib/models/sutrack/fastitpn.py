...
class Attention(nn.Module):
    def __init__(
            ...
            use_linear_attn=False,  # 新增：是否使用线性注意力
    ):
        super().__init__()
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.use_linear_attn:
            ...
            # 线性注意力核心：改变计算顺序 O(N^2) -> O(N)
            # 标准: (Q @ K.T) @ V  -> 先算 N×N 注意力矩阵
            # 线性: Q @ (K.T @ V)  -> 先算 d×d 矩阵，与N无关
            
            # 计算 K.T @ V: (B, heads, N, d) @ (B, heads, N, d) -> (B, heads, d, d)
            # 注意：k需要转置，einsum中n是序列维度，d是特征维度
            # k: [B, h, N, d], v: [B, h, N, d] -> kv: [B, h, d, d]
            kv = torch.einsum('bhnd,bhne->bhde', k, v)
            
            # 归一化因子：保证数值稳定性
            # 分母: sum_j φ(q_i)^T φ(k_j) = φ(q_i)^T * sum_j(φ(k_j))
            k_sum = k.sum(dim=2)  # (B, heads, d) - 对序列维度N求和
            # 防止k_sum为0
            k_sum = torch.clamp(k_sum, min=1e-6)
            
            # z: [B, h, N] = 1 / (q[B,h,N,d] @ k_sum[B,h,d])
            z = 1 / (torch.einsum('bhnd,bhd->bhn', q, k_sum) + 1e-6)
            # 防止z出现inf或nan
            z = torch.clamp(z, max=1e6)
            
            # 最终输出: φ(Q) @ (K.T @ V) / 归一化
            # Q @ KV: (B, heads, N, d) @ (B, heads, d, d) -> (B, heads, N, d)
            ...
            # 注意：线性注意力不支持相对位置编码（因为没有显式的N×N矩阵）
            # 位置信息通过pos_embed提供
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            ...
            deepnorm=deepnorm,
            subln=subln,
            use_linear_attn=use_linear_attn,  # 传递给Attention
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.build_blocks(
            ...
            convmlp=convmlp,
            use_linear_attn=use_linear_attn,  # 传递给build_blocks
        )
        ...
        if self.motion_token is not None:
    ...
    def build_blocks(self,
                     ...
                     naiveswiglu=False,
                     convmlp=False,
                     use_linear_attn=False,  # 新增：Stage 3是否使用线性注意力
                     ):
        ...
        ######### stage 3 ########
        # Stage 3: 使用线性注意力优化计算复杂度 O(N^2) -> O(N)
        self.blocks.extend([
            Block(
                dim=dims['16'],
                num_heads=num_heads,
                ...
                swiglu=swiglu,
                naiveswiglu=naiveswiglu,
                use_linear_attn=use_linear_attn,  # 启用线性注意力
    ...
    def _compute_template_velocity(self, template_anno_list):
        B = template_anno_list[0].size(0)
        device = template_anno_list[0].device
        if len(template_anno_list) < 2:
            return torch.zeros(B, 2, device=device)
        c0 = template_anno_list[0][:, :2] + 0.5 * template_anno_list[0][:, 2:4]
        c1 = template_anno_list[1][:, :2] + 0.5 * template_anno_list[1][:, 2:4]
        return c1 - c0
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        if self.motion_token is not None:
            velocity = self._compute_template_velocity(template_anno_list)
            vel_embed = self.velocity_encoder(velocity)
            mot_tokens = self.motion_token.expand(B, -1, -1) + vel_embed.unsqueeze(1)
            xz = torch.cat([xz, mot_tokens], dim=1)
...
def load_pretrained(model, checkpoint, pos_type, patchembed_init):
    ...
    pretrain_in_chans = patch_embedding_weight_pretrained.shape[1]
    if model_in_chans == pretrain_in_chans:
        state_dict['patch_embed.proj.weight'] = patch_embedding_weight_pretrained
    elif patchembed_init == "copy":
        patch_embedding_weight[:,:pretrain_in_chans,:,:] = patch_embedding_weight_pretrained
        patch_embedding_weight[:,pretrain_in_chans:,:,:] = patch_embedding_weight_pretrained
    elif patchembed_init == "halfcopy":
        patch_embedding_weight[:,:pretrain_in_chans,:,:] = patch_embedding_weight_pretrained / 2
        patch_embedding_weight[:,pretrain_in_chans:,:,:] = patch_embedding_weight_pretrained / 2
...
def fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    in_chans = kwargs.pop("in_chans", 6)
    model = Fast_iTPN(
        patch_size=16, in_chans=in_chans, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=12, num_heads=6, bridge_mlp_ratio=3.,
...
def fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    in_chans = kwargs.pop("in_chans", 6)
    model = Fast_iTPN(
        patch_size=16, in_chans=in_chans, embed_dim=384, depth_stage1=2, depth_stage2=2, depth=20, num_heads=6, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
...
def fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", 
              use_linear_attn=False, use_motion_token=False, **kwargs):
    """
    Fast-iTPN Base模型
    
    Args:
        use_linear_attn: 是否使用线性注意力替代标准注意力
        use_motion_token: 是否使用运动编码token (MET)
    """
    in_chans = kwargs.pop("in_chans", 6)
    model = Fast_iTPN(
