...
Support Fast-iTPN with LAST-ViT frequency domain selection.
"""

from torch import nn
from lib.utils.misc import is_main_process
from lib.models.sutrack import fastitpn as fastitpn_module
from lib.models.sutrack import fastitpn_lastvit as fastitpn_lastvit_module
        ...
        for name, parameter in encoder.named_parameters():
            ...
            if not train_encoder:
                ...
                for open_block in open_blocks:
                    if str(open_block) in name:  # 将整数转换为字符串
                        freeze = False
...
class Encoder(EncoderBase):
    """ViT encoder with LAST-ViT support."""
    def __init__(self, name: str,
                 ...
                 open_layers: list,
                 cfg=None):
        if "lastvit" in name.lower():
            # LAST-ViT 版本：使用频域选择
            encoder = getattr(fastitpn_lastvit_module, name)(
                ...
                in_chans=cfg.MODEL.ENCODER.IN_CHANS,
                # LAST-ViT 特定参数
                use_lastvit_selection=getattr(cfg.MODEL.ENCODER, 'USE_LASTVIT_SELECTION', True),
                select_ratio=getattr(cfg.MODEL.ENCODER, 'SELECT_RATIO', 0.7),
                freq_kernel_size=getattr(cfg.MODEL.ENCODER, 'FREQ_KERNEL_SIZE', 7),
                freq_sigma=getattr(cfg.MODEL.ENCODER, 'FREQ_SIGMA', 2.0),
                use_adaptive_selection=getattr(cfg.MODEL.ENCODER, 'USE_ADAPTIVE_SELECTION', False),
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
            ...
            encoder = getattr(fastitpn_module, name)(
                ...
                patchembed_init = cfg.MODEL.ENCODER.PATCHEMBED_INIT,
                in_chans = cfg.MODEL.ENCODER.IN_CHANS,
                use_linear_attn=getattr(cfg.MODEL.ENCODER, 'USE_LINEAR_ATTN', False),
                use_motion_token=getattr(cfg.MODEL.ENCODER, 'USE_MOTION_TOKEN', False),
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
