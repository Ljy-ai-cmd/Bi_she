...
class SUTrack_Actor(BaseActor):
    ...
    def __init__(self, net, objective, loss_weight, settings, cfg):
        ...
        self.use_motion_temporal = getattr(cfg.MODEL, 'USE_MOTION_TEMPORAL', False)
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        motion_smooth_loss = torch.tensor(0.0, device=l1_loss.device)
        if self.use_motion_temporal:
            pred_center = pred_boxes[:, 0, 0:2]
            template_anno = gt_dict['template_anno']
            if len(template_anno) >= 2:
                t0_center = template_anno[0][:, 0:2] + 0.5 * template_anno[0][:, 2:4]
                t1_center = template_anno[1][:, 0:2] + 0.5 * template_anno[1][:, 2:4]
                velocity = t1_center - t0_center
                expected_center = t1_center + velocity
                motion_smooth_loss = F.smooth_l1_loss(pred_center, expected_center, reduction='mean')

        # weighted sum
        motion_weight = getattr(self.cfg.TRAIN, 'MOTION_SMOOTH_WEIGHT', 0.1)
        loss = (self.loss_weight['giou'] * giou_loss +
                self.loss_weight['l1'] * l1_loss +
                self.loss_weight['focal'] * location_loss +
                self.loss_weight['task_cls'] * task_cls_loss +
                motion_weight * motion_smooth_loss)

        if return_status:
            # status for log
            mean_iou = iou.detach().mean()
            search_size = float(self.cfg.DATA.SEARCH.SIZE)
            pred_center = pred_boxes[:, 0, 0:2].detach() * search_size
            gt_center = (gt_bbox[:, 0:2] + 0.5 * gt_bbox[:, 2:4]).detach() * search_size
            center_dist = torch.norm(pred_center - gt_center, dim=1)
            precision = (center_dist <= 20.0).float().mean()
            iou_per_sample = iou.detach().view(-1, num_queries).max(dim=1)[0]
            recall = (iou_per_sample >= 0.5).float().mean()
            status = {"Loss/total": loss.item(),
                      "Loss/giou": giou_loss.item(),
                      "Loss/l1": l1_loss.item(),
                      "Loss/location": location_loss.item(),
                      "Loss/task_class": task_cls_loss.item(),
                      "Loss/motion_smooth": motion_smooth_loss.item(),
                      "IoU": mean_iou.item(),
