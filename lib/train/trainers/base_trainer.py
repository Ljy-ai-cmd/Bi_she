...
class BaseTrainer:
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        if not hasattr(self, "best_val_epoch"):
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if getattr(self.settings, "save_best", True) and "val" in self.stats and self.stats["val"] is not None:
                        val_stats = self.stats["val"]
                        if "Loss/total" in val_stats and hasattr(val_stats["Loss/total"], "history") and val_stats["Loss/total"].history:
                            val_loss = float(val_stats["Loss/total"].history[-1])
                            if val_loss < self.best_val_loss:
                                self.best_val_loss = val_loss
                                self.best_val_epoch = epoch
                                if self._checkpoint_dir and self.settings.local_rank in [-1, 0]:
                                    self.save_checkpoint_best()
    ...
    def save_checkpoint_best(self):
        ...
        state = {
            ...
            'best_val_epoch': getattr(self, "best_val_epoch", None),
        }
        ...
        tmp_file_path = '{}/{}_best.tmp'.format(directory, net_type)
        torch.save(state, tmp_file_path)
        file_path = '{}/{}_best.pth.tar'.format(directory, net_type)
