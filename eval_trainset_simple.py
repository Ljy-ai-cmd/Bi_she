#!/usr/bin/env python3
"""
SUTrack Training Set Evaluation Script
在Anti-UAV训练集上评估模型性能（红外模态）
...
def main():
    """主函数 - 训练集评估"""
    # 权重路径 - 使用指定的权重
    MODEL_PATH = r'E:\biyesheji\SUTrack-main11\checkpoints\train\sutrack\sutrack_b224\SUTRACK_ep0180.pth.tar'
    CONFIG_NAME = 'sutrack_b224'
    # 使用训练集而不是测试集
    TRAIN_DIR = r'E:\biyesheji\SUTrack-main11\data\AntI-UAV\train'
    
    print("="*60)
    print("SUTrack Training Set Evaluation (Infrared + SA Algorithm)")
    print("="*60)
    ...
    # 创建跟踪器
    tracker = SUTRACK(params, 'ANTIUAV')
    
    # 查找所有训练序列
    print("\nLoading train sequences...")
    sequences = []
    for seq_name in sorted(os.listdir(TRAIN_DIR)):
    ...
    print(f"Found {len(sequences)} train sequences with infrared data")
    ...
    if total_sa:
        ...
        print("\n" + "="*60)
        print("Evaluation Results - Training Set (Infrared Modality with SA Algorithm)")
        print("="*60)
        ...
        print(f"Sequences:           {len(all_results)}")
        print("="*60)
        ...
        output_file = 'evaluation_results_train_sa_infrared.json'
