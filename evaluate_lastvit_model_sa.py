#!/usr/bin/env python3
"""
SUTrack LAST-ViT Model Evaluation Script with SA Algorithm
在Anti-UAV测试集上评估 LAST-ViT 模型性能（红外模态）
...
def load_frame_pair(seq_dir, frame_idx):
    """加载一对可见光和红外帧，合并成6通道"""
    # 尝试不同的命名格式
    naming_styles = [
        (f'visible/visibleI{frame_idx:04d}.jpg', f'infrared/infraredI{frame_idx:04d}.jpg'),
        (f'visible/{frame_idx:06d}.jpg', f'infrared/{frame_idx:06d}.jpg'),
        (f'visible/{frame_idx+1:06d}.jpg', f'infrared/{frame_idx+1:06d}.jpg'),
    ]
    
    for vis_name, ir_name in naming_styles:
        vis_path = os.path.join(seq_dir, vis_name)
        ir_path = os.path.join(seq_dir, ir_name)
        
        if os.path.exists(vis_path) and os.path.exists(ir_path):
            rgb = cv2.imread(vis_path)
            ...
            if rgb is not None and ir is not None:
                ...
                # 合并成6通道 - 尝试 (IR, RGB) 顺序
                return np.concatenate((ir, rgb), axis=2)
    
    return None


def evaluate_infrared_sequence_fast(tracker, seq_dir, gt_boxes, exist_flags):
    """评估单个视频序列 - 使用真实的双模态数据（RGB+IR）"""
    import cv2
    
    # 检查是否有图像序列
    visible_dir = os.path.join(seq_dir, 'visible')
    infrared_dir = os.path.join(seq_dir, 'infrared')
    
    use_images = os.path.isdir(visible_dir) and os.path.isdir(infrared_dir)
    
    # 如果可用，使用视频文件
    infrared_path = os.path.join(seq_dir, 'infrared.mp4')
    use_video = os.path.exists(infrared_path)
    
    if not use_images and not use_video:
        return None
    ...
    # 读取初始化帧
    if use_images:
        frame_rgb = load_frame_pair(seq_dir, init_frame)
        if frame_rgb is None and use_video:
    ...
    # 批量读取和跟踪
    frame_idx = 1
    current_frame = init_frame + 1
    
    while frame_idx < max_frames and current_frame < total_frames:
        # 优先使用图像序列（真实的双模态）
        if use_images:
            frame_rgb = load_frame_pair(seq_dir, current_frame)
            if frame_rgb is None and use_video:
                ret, frame = cap.read()
                if ret:
                    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
...
def main():
    """主函数 - LAST-ViT模型评估"""
    MODEL_PATH = r'E:\biyesheji\SUTrack-main11\output\lastvit_gpu2_full\checkpoints\train\sutrack\sutrack_b224_antiuav_lastvit\SUTRACK_ep0180.pth.tar'
    CONFIG_NAME = 'sutrack_b224_antiuav_lastvit'
    TEST_DIR = r'E:\biyesheji\SUTrack-main11\data\AntI-UAV\train'
    
    print("="*60)
    print("SUTrack LAST-ViT Model Evaluation (Infrared + SA Algorithm)")
    print("="*60)
    ...
    if total_sa:
        ...
        print("\n" + "="*60)
        print("Evaluation Results - LAST-ViT (Infrared + SA Algorithm)")
        print("="*60)
        ...
        output_file = 'evaluation_results_lastvit_sa.json'
